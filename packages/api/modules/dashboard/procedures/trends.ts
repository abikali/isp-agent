...
export const getDashboardTrends = protectedProcedure
	...
	.handler(async ({ context: { user }, input }) => {
		...
		const customerDealerFilter = getDealerScopeFilter(activeDealerId);
		const paymentDealerFilter = getDealerScopeViaCustomer(activeDealerId);
		...
		for (const p of payments) {
			...
			revenueByDay.set(day, (revenueByDay.get(day) ?? 0) + p.paidAmount);
		}
		...
		for (const c of customers) {
			...
			const daysOverdue = Math.floor(
				(now.getTime() - c.expiresAt.getTime()) / (1000 * 60 * 60 * 24),
			);
