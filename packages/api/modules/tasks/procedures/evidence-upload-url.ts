...
export const createEvidenceUploadUrl = protectedProcedure
	.route({
		method: "POST",
		path: "/tasks/evidence-upload-url",
		tags: ["Tasks"],
		summary: "Get a signed upload URL for task evidence photos",
	})
	.input(
		z.object({
	...
	.handler(async ({ context: { user }, input }) => {
		...
		const storagePath = `task-evidence/${input.organizationId}/${fileId}.${ext}`;
