		...
		switch (event.type) {
			case "order.created": {
				...
				if (metadata?.["organization_id"]) {
					entityInfo1.organizationId = metadata[
						"organization_id"
					] as string;
				}
				if (metadata?.["user_id"]) {
					entityInfo1.userId = metadata["user_id"] as string;
				}
				await setCustomerIdToEntity(customerId, entityInfo1);
			...
			case "subscription.created": {
				...
				if (metadata?.["organization_id"]) {
					entityInfo2.organizationId = metadata[
						"organization_id"
					] as string;
				}
				if (metadata?.["user_id"]) {
					entityInfo2.userId = metadata["user_id"] as string;
				}
				await setCustomerIdToEntity(customerId, entityInfo2);
