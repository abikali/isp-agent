...
export const createCheckoutLink: CreateCheckoutLink = async (options) => {
	...
	if (redirectUrl) {
		productOptions.redirectUrl = redirectUrl;
	}
		...
		switch (eventName) {
			case "subscription_created": {
				...
				if (customData.organization_id) {
					entityInfo1.organizationId = customData.organization_id;
				}
				if (customData.user_id) {
					entityInfo1.userId = customData.user_id;
				}
			...
			case "order_created": {
				...
				if (customData.organization_id) {
					entityInfo2.organizationId = customData.organization_id;
				}
				if (customData.user_id) {
					entityInfo2.userId = customData.user_id;
				}
				await setCustomerIdToEntity(
					String(data.attributes.customer_id),
					entityInfo2,
