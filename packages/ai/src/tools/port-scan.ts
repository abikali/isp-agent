...
function createPortScanTool() {
	return tool({
		...
		execute: async ({ host, ports, preset }) => {
			const hostError = validateHost(host);
			if (hostError) {
				return { success: false, message: hostError };
			}

			const portsToScan = ports ?? PORT_PRESETS[preset ?? ""] ?? [];
			...
			const results = await Promise.all(
				portsToScan.map((port) => checkPort(host, port, 3000)),
			);
			...
			return {
				success: true,
				message: `Port scan of ${host}: ${openCount}/${results.length} ports open`,
				results,
			};
		},
	});
