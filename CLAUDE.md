...
#### Customer expiry: live `Customer.expiresAt` vs frozen `CustomerInvoice.expiryDate`

There are two notions of "expiry", and they are **not interchangeable**. The frozen billing deadline lives **per-invoice**, not on the customer.

> **History:** there used to be a `Customer.billingExpiresAt` snapshot field. It was retired on 2026-04-21 (commit `0cf7104`, migration `20260421230000_drop_billing_expires_at`) in favor of the per-invoice `expiryDate` below. If you see `billingExpiresAt` referenced anywhere, it's stale — the column no longer exists.

**`Customer.expiresAt`** — live mirror of iRadius `UserNas.ExpiryAccount`.
- Written by iRadius sync. Tracked as a conflict field (`iradius-sync-fields.ts`) — if iRadius differs from local, a `sync_conflict` row is created rather than overwriting silently.
- Also writable from specific admin flows (e.g., unstop/reactivation in `stopped.ts`, which writes `expiresAt`).
- **Read it for "what does iRadius say right now?"** — customer detail "Expires" field, or "service has lapsed on iRadius" ops filters. This is the *current* truth, which can shift between syncs.

**`CustomerInvoice.expiryDate`** — frozen per-month billing deadline (`schema.prisma`, model `CustomerInvoice`).
- Set once at invoice generation time: `generate-invoices.ts` writes `expiryDate: customer.expiresAt ?? range.lte` (the customer's live expiry at generation, falling back to the end of the billing month).
- Frozen thereafter — never rewritten by iRadius sync. The next month's invoices are generated by `toggleMonthLock`, whose sync-age guard guarantees `expiresAt` was freshly pulled just before the snapshot.
- **The invoice is the unit of truth for collection.** `list-unpaid` and the other collector-facing readers join `customer_invoice` and read each invoice's own `expiryDate` (see `list-unpaid.ts` — `inv.expiryDate` drives `oldestUnpaidExpiry`).

**Why per-invoice instead of a customer-level snapshot?** iRadius can mutate `ExpiryAccount` mid-cycle (auto-renew batches, admin tweaks). If collectors' lists read the live value, the "who owes money" list shifts under them between syncs. Freezing the deadline on each `customer_invoice` row keeps every month's billing picture stable on its own row — and unlike the old single customer-level field, history for past months is preserved per-invoice instead of being overwritten each lock.

**Rules:**
- Any new query in `packages/api/modules/billing/**` or collector-facing UI must read the invoice's `expiryDate` (join `customer_invoice`), **not** the live `customer.expiresAt`.
- Non-billing "is this customer expired on iRadius?" checks use the live `customer.expiresAt`.
- Invoice rows are the refresh point: `toggleMonthLock` → `generateInvoicesForMonth` freezes the new month's `expiryDate`. Don't backfill or rewrite `expiryDate` on existing invoices without understanding the freeze semantics.

**Known gap (not bug — documented behavior):** the unpaid-months walk in `list-unpaid.ts` can count a customer as owing for months they were stopped and later reactivated, since it doesn't cross-check that a real `customer_invoice` row existed for each intervening month. Cross-reference `customer_invoice` by `(year, month)` if that matters for your use case.
...
- **Read-only rule (ad-hoc access)**: NEVER hand-modify the iRadius MySQL database from a shell/session — it is production and actively used. Investigate read-only; always `DESCRIBE tablename` before querying; BIT fields require `toBooleanFromBit()`. This governs *ad-hoc* access. The app itself **does** write to iRadius, but only through the sanctioned helpers below (`mirrorToIRadius`, `pushMirrorDiffToIRadius`, `iradius-api.ts`, and for dealer money `dealers/lib/iradius-dealer.ts` — `iradiusAdjustDealerCredit` / `iradiusRecordDealerPayment`, which replicate the legacy `DealerConfiguration.manageDealerAccount` statements in one transaction) — if a data correction is needed, run it through those helpers rather than raw SQL, so the same validation and local-DB write happen.
...
- **Creating a subscriber in iRadius (worker-request approval)**: `approveSetupRequest` → `createCustomerInIRadius` (`customers/lib/create-in-iradius.ts`) does three remote calls in order, and the order matters:
  1. `iradiusCreateUser` — our patched `/create-user` endpoint. Inserts `User` + `UserNas` only; **no billing**. Writes `UserNas.ExpiryAccount` verbatim from `customer.expiresAt`.
  2. `iradiusChargeNewUser` — the servlet we added, which calls iRadius's own `RenewUser.renewUser(NEW USER)`. This is what debits the dealer's credit and generates the opening invoice, so it must not be removed. **It also rewrites `ExpiryAccount`**: `ChangeAccountTypeDao.updateUserExpiryAccountDate` takes the *current* expiry as its base (it only falls back to "now" when the account type sets `ImmediateRecharge`, which LibanCom's don't) and adds `AccountType.ValidityPeriod` — a full month — normalising the time to `23:55`.
  3. `iradiusSetExpiryAccount` — re-asserts the intended expiry, undoing step 2's extra month. **Do not drop this**, or every approved customer gets a free extra month (a 15-day setup expiring at +1m15d, a "1 month" one at +2 months). It is log-and-continue: throwing after step 1 would abort the approval before `externalId` is stored locally, orphaning a subscriber that blocks every retry on "username already exists".

  The charge writes its own `UserLog` row (`Description` = `LibanCom App … [Expiry Date = …]`), which is the audit trail for what any given approval actually set.
